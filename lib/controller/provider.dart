...
class ProviderController extends GenericController {
	...
	Future create(Provider item) => _opendaf.dafman.create(_prefix, item.name, item.toCfgJson()).then((_) {
		item.cfg_stash();
		_opendaf.root.providers[item.name] = item;
		_opendaf.root.eventController.add(new ProvidersSetChanged());
	});
	Future update(Provider item) => _opendaf.dafman.update(_prefix, item.name, item.toCfgJson()).then((_) {
		_opendaf.root.providers[item.name] = item;
		_opendaf.root.eventController.add(new ProvidersSetChanged());
	});
