...
class CommandController extends GenericController {
	...
	Future create(Command item) => _opendaf.dafman.create(_prefix, item.name, item.toCfgJson()).then((_) {
		item.cfg_stash();
		_opendaf.root.commands[item.name] = item;
		_opendaf.root.eventController.add(new CommandsSetChanged());
	});
	Future update(Command item) => _opendaf.dafman.update(_prefix, item.name, item.toCfgJson()).then((_) {
		_opendaf.root.commands[item.name] = item;
		_opendaf.root.eventController.add(new CommandsSetChanged());
	});
