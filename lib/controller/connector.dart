...
class ConnectorController extends GenericController {
	...
	Future create(Connector item) => _opendaf.dafman.create(_prefix, item.name, item.toCfgJson()).then((_) {
		item.cfg_stash();
		_opendaf.root.connectors[item.name] = item;
		_opendaf.root.eventController.add(new ConnectorsSetChanged());
	});
	Future update(Connector item) => _opendaf.dafman.update(_prefix, item.name, item.toCfgJson()).then((_) {
		_opendaf.root.connectors[item.name] = item;
		_opendaf.root.eventController.add(new ConnectorsSetChanged());
	});
